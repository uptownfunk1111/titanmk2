...
# --- INCORPORATE CONFIDENCE & RISK ---
try:
    from utilities.prediction_confidence import add_confidence
except ImportError:
    import sys
    import importlib.util
    conf_path = os.path.join(os.path.dirname(__file__), 'utilities', 'prediction_confidence.py')
    spec = importlib.util.spec_from_file_location('prediction_confidence', conf_path)
    prediction_confidence = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(prediction_confidence)
    add_confidence = prediction_confidence.add_confidence

conf_output_path = os.path.join(outputs_dir, f'this_weeks_tips_confidence_{datetime.now().date()}.csv')
try:
    add_confidence(output_path, conf_output_path)
except Exception as e:
    print(f"[WARN] Could not add confidence/risk: {e}")
...
if __name__ == "__main__":
    ...
    if args.speculative:
        ...
        subprocess.run([sys.executable, os.path.join("utilities", "speculative_data_sweep.py")])
